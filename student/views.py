...
def studentLogin(request):
    if request.method=='POST':
        ...
        if password==student.password:  #登录成功
            ...
            # 渲染index模板
            return render(request,'stupage.html',{'student':student,'paper':paper,'grade':grade})
        else:
            id!=stuId
            return render(request,'xxx.html',{'message':'密码不正确'})
...
def teacherLogin(request):
    ...
    if request.method == 'POST':
        ...
        if password == teacher.password:  # 登录成功
            ...
            data1=models.Grade.objects.filter(subject='电力拖动',grade__lt=60).count()
            data2=models.Grade.objects.filter(subject='电力拖动',grade__gte=60,grade__lt=70).count()
            data3 = models.Grade.objects.filter(subject='电力拖动', grade__gte=70, grade__lt=80).count()
            data4 = models.Grade.objects.filter(subject='电力拖动', grade__gte=80, grade__lt=90).count()
            data5 = models.Grade.objects.filter(subject='电力拖动', grade__gte=90).count()

            data6 = models.Grade.objects.filter(subject='数学', grade__lt=60).count()
            data7= models.Grade.objects.filter(subject='数学', grade__gte=60, grade__lt=70).count()
            data8= models.Grade.objects.filter(subject='数学', grade__gte=70, grade__lt=80).count()
            data9 = models.Grade.objects.filter(subject='数学', grade__gte=80, grade__lt=90).count()
            data10= models.Grade.objects.filter(subject='数学', grade__gte=90).count()
